...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback):
    ...
    async_add_entities([
        AutonomicMute(client, zone, names[i])
        for i, zone in enumerate(client.zones)
    ])

class AutonomicMute(SwitchEntity):
    ...
    def __init__(self, client, zone, room_name):
        ...
        self._attr_unique_id = f"autonomic_m401_zone_{zone}_mute"
    ...
    @property
    def is_on(self):
        return self._client.state[self._zone]["mute"]

    async def async_turn_on(self, **kwargs):
        await self._client.set_mute(self._zone, True)
        self.async_write_ha_state()

    async def async_turn_off(self, **kwargs):
        await self._client.set_mute(self._zone, False)
        self.async_write_ha_state()
